...
def main_eval(env, model_dir, exp_name, model_name):
    ...
    while not done:
        ...
        if i % 5 == 0:
            frames.append(frame)
        actions, lstm_states = model.predict(obs, state=lstm_states, episode_start=episode_starts, deterministic=True)
